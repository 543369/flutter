...
class DashboardController extends ApiSupport {
 ...
 Map<String,Object> dashboard(Authentication auth) {
  ...
  var pets = db.queryForList("SELECT id,name,species FROM pets WHERE household_id = ? ORDER BY name,id", home);
  ...
  var history = db.query("SELECT e.id,e.task_id,e.action,e.happened_at,a.display_name,t.title,p.name FROM care_events e JOIN care_tasks t ON t.id=e.task_id JOIN pets p ON p.id=t.pet_id LEFT JOIN accounts a ON a.id=e.actor_id WHERE p.household_id=? ORDER BY e.happened_at DESC,e.id DESC LIMIT 100", (r,n) -> {
   Map<String,Object> row = new LinkedHashMap<>();
   ...
   row.put("title",r.getString(6)); row.put("petName",r.getString(7)); return row;
  }, home);
  var recurring = db.queryForList("SELECT c.id,c.title,c.frequency,c.zone_id AS zoneId,c.active,p.name AS petName FROM care_plans c JOIN pets p ON p.id=c.pet_id WHERE p.household_id=? ORDER BY c.active DESC,c.title,c.id",home);
