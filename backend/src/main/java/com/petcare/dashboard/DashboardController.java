...
class DashboardController extends ApiSupport {
 ...
 Map<String,Object> dashboard(Authentication auth) {
  ...
  var pets = db.query("SELECT id,name,species,photo_data FROM pets WHERE household_id = ? ORDER BY name,id", (r,n) -> {
   Map<String,Object> row = new LinkedHashMap<>();
   row.put("id",r.getString(1)); row.put("name",r.getString(2)); row.put("species",r.getString(3));
   row.put("photoData",r.getString(4)); return row;
  }, home);
  plans.materialize(home);
  var tasks = db.query("SELECT t.id,t.pet_id,t.title,t.due_at,t.completed,p.name,t.plan_id FROM care_tasks t JOIN pets p ON p.id=t.pet_id WHERE p.household_id=? AND t.cancelled=FALSE ORDER BY t.completed,t.due_at,t.id", (r,n) -> {
   Map<String,Object> row = new LinkedHashMap<>();
   ...
   row.put("petName",r.getString(6)); row.put("planId",r.getString(7)); return row;
  }, home);
  var history = db.query("SELECT e.id,e.task_id,e.action,e.happened_at,a.display_name,t.title,p.name,p.id FROM care_events e JOIN care_tasks t ON t.id=e.task_id JOIN pets p ON p.id=t.pet_id LEFT JOIN accounts a ON a.id=e.actor_id WHERE p.household_id=? ORDER BY e.happened_at DESC,e.id DESC LIMIT 100", (r,n) -> {
   Map<String,Object> row = new LinkedHashMap<>();
   ...
   row.put("title",r.getString(6)); row.put("petName",r.getString(7)); row.put("petId",r.getString(8)); return row;
  }, home);
  var recurring = db.queryForList("SELECT c.id,c.title,c.frequency,c.zone_id AS zoneId,c.active,p.name AS petName,p.id AS petId FROM care_plans c JOIN pets p ON p.id=c.pet_id WHERE p.household_id=? ORDER BY c.active DESC,c.title,c.id",home);
